...
export default function PersonDetail() {
  ...
  const hasLegislativeRole = memberships.some(m => isLegislativeRole(m.chamber));

  // Calculate total executive orders signed across all terms
  const totalExecutiveOrdersSigned = documents.filter(d =>
    d.type === 'executive_order' &&
    (d as any).mayor_id === person.id
  ).length;
  ...
  const recentLegislation = (hasExecutiveRole && !hasLegislativeRole
            ...
            <div className='flex items-center gap-2'>
              ...
              <span
                className={`text-3xl leading-none font-black ${
                  ...
                  overallAttendanceRate >= 75 ? 'text-secondary-600' :
                  'text-red-600'
                }`}
              >
            ...
            <div className='space-y-3'>
              {membershipsWithDetails.map((membership, index) => {
                ...
                const committeesByRole = {
                  chairperson: [] as Array<{ id: string; role: string; name: string }>,
                  viceChairperson: [] as Array<{ id: string; role: string; name: string }>,
                  member: [] as Array<{ id: string; role: string; name: string }>,
                };
                (membership.committees || []).forEach(c => {
                  ...
                  const normalizedRole = c.role.toLowerCase().replace(/[^a-z]/g, '');
                  const roleKey = normalizedRole.includes('vicechair')
                    ? 'viceChairperson'
                    : normalizedRole.includes('chair')
                    ? 'chairperson'
                    : 'member';
                  committeesByRole[roleKey].push({ id: c.id, role: c.role, name: displayName });
                });
                // Sort committees within each group alphabetically
                Object.values(committeesByRole).forEach(group => group.sort((a, b) => a.name.localeCompare(b.name)));
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <div className='absolute right-0 top-0 rounded-bl-xl bg-primary-600 px-3 py-1 text-[10px] font-bold text-white'>
                        CURRENT TERM
                      </div>
                    )}
                      ...
                      <div className='flex-1 space-y-4'>
                        ...
                        <div className='flex flex-wrap items-center gap-2'>
                          ...
                          <Badge variant='secondary' className='text-xs'>
                            {membership.role}
                            {membership.rank !== undefined && ` • #${membership.rank}`}
                          </Badge>
                        </div>
                          ...
                          <div className='space-y-3'>
                            ...
                            <div className='flex items-center gap-2 border-b border-slate-100 pb-2'>
                              ...
                              <span className='ml-auto text-xs text-slate-400'>
                                {committeesByRole.chairperson.length + committeesByRole.viceChairperson.length + committeesByRole.member.length} total
                              </span>
                            </div>
                          ...
                          {!isExecutive ? (
                            <>
                              ...
                              <span className='text-slate-600'>
                                <span className='font-semibold text-primary-600'>{membership.termStats?.ordinances || 0}</span> ord,
                                <span className='font-semibold text-secondary-600'> {membership.termStats?.resolutions || 0}</span> res
                                {membership.termStats?.executiveOrders ? (
                                  <span>, <span className='font-semibold text-purple-600'>{membership.termStats.executiveOrders}</span> EO</span>
                                ) : ''}
                              </span>
                            </>
                          ) : (
                            <span className='text-slate-600'>
                              <span className='font-semibold text-purple-600'>{membership.executiveOrdersSigned || 0}</span> Executive Orders signed
                            </span>
                          )}
                        </div>
                      </div>
                ...
                return (
                  ...
                  >
                    <div className='flex items-start gap-3'>
                      ...
                      <ChevronRight className='h-5 w-5 shrink-0 text-slate-300 group-hover:text-primary-600 transition-colors mt-0.5' />
                    </div>
                  </Link>
                );
              }) : (
            ...
            >
              <div className='space-y-3'>
                <p className='text-sm text-slate-600'>
                  As {latestMembership?.role || 'Executive'}, this official has signed {totalExecutiveOrdersSigned} executive order{totalExecutiveOrdersSigned !== 1 ? 's' : ''} across their term{totalTermsServed > 1 ? 's' : ''}.
                </p>
