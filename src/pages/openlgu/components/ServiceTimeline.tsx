...
export default function ServiceTimeline({
  ...
  const countDocumentsForTerm = (termId: string): number => {
    return documents.filter(doc => doc.term_id === termId && doc.author_ids.includes(person.id)).length;
  };

  // Count executive orders signed as mayor for a specific term
  const countExecOrdersForTerm = (termId: string): number => {
    return documents.filter(
      doc => doc.term_id === termId && doc.type === 'executive_order' && doc.mayor_id === person.id
    ).length;
  };

  // Get role badge variant based on role
  const getRoleVariant = (role: string): 'primary' | 'secondary' | 'warning' | 'slate' => {
      ...
      {sortedMemberships.map(membership => {
        ...
        return (
          <div
            ...
            className="relative pl-6 border-l-2 border-slate-200 pb-4 last:pb-0 last:border-0"
          >
            {/* Timeline dot */}
            <div className="absolute left-0 top-1.5 h-3 w-3 -translate-x-[5px] rounded-full border-2 border-primary-500 bg-white" />

            <div className="space-y-2">
              {/* Term header */}
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm font-bold text-slate-900">{term.year_range}</span>
              </div>
